...
{
    public class CustomROCDateValidator : CustomDependepntValidator
    {
        ...
        protected override bool CheckCustomLogic()
        {
            ...
            if (controlToValidate.Length > 0)
            {
                ...
                {
                    if(!ValidationHelper.ValidateROCDateFormat(controlValue))
                    {

                        if (string.IsNullOrWhiteSpace(FormatErrorMsg))
                        {
                            this.ErrorMessage = "請輸入YYY/MM/DD格式之日期";
                        }
                        else
                        {
